...
final class PlaybackStream {
    ...
    static String urlFor(Models.MediaItem item) {
        return urlFor(item, null);
    }

    static String urlFor(Models.MediaItem item, String hlsSessionId) {
        if (item == null) {
            return null;
        }
        ...
        if (shouldUseCompatibleStream(item)) {
            ...
            if (compatible != null) {
                return hlsCompatibleUrl(compatible, item, hlsSessionId);
            }
        }
    ...
    static String hlsFallbackUrl(Models.MediaItem item, String currentUrl) {
        return hlsFallbackUrl(item, currentUrl, null);
    }

    static String hlsFallbackUrl(Models.MediaItem item, String currentUrl, String hlsSessionId) {
        ...
        String hls = hlsCompatibleUrl(compatible, item, hlsSessionId);
    ...
    static boolean lacksSeekableDuration(boolean windowSeekable, long windowDurationMs) {
        return !windowSeekable || windowDurationMs <= 0L;
    }
    ...
    static String hlsCompatibleUrl(String url, Models.MediaItem item) {
        return hlsCompatibleUrl(url, item, null);
    }

    static String hlsCompatibleUrl(String url, Models.MediaItem item, String hlsSessionId) {
        ...
        if (hlsSessionId != null && !hlsSessionId.isEmpty()
                && !queryContains(converted, "session", null)) {
            converted = withQueryParam(converted, "session", hlsSessionId);
        }
        return converted;
    }
